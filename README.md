...
Long-press Facebook's icon on your home screen or in your app list and tap **Hushfacebook**, the first item in that menu. It's the app icon, not the Facebook logo inside the app. If the item isn't there, open Facebook once so it can add it, then try again. Related controls sit in rounded groups, with the build's status at the top. The screen lists the features this build carries:
...
- **Video file name**, the name each saved video gets. It starts as `FB_VID_{date}`, Facebook's own naming, so nothing changes until you edit it. `{date}` becomes the date and time of the save, like `20260925_143005`, and `{video_id}` the video's number on Facebook. When a save doesn't know the number, it's left out. Android only numbers a repeated name so many times before it refuses the save, so a name with neither token gets `_{date}` added, and one that counts on a missing number gets the date and time on the end. The name is cleaned the way the folder is. A typed extension such as `.mp4` is dropped, since the saved file's type decides it, and photos keep Facebook's `FB_IMG_` names, which a video's name can't start with.
...
Morphe Manager can export your patch choices and your signing key, but not the switches on this screen. **Export settings** saves them, with the download quality, the save folder and the video file name, to a JSON file wherever you pick, and **Import settings** reads one back, on this phone or a new one. Before anything changes, the screen says how many switches the file would change, what it does to the download settings and how many entries in it this version doesn't know, which it skips. A damaged file or one from a newer Hushfacebook changes nothing. Pause and Debug logging stay out of the file, and so does anything about you or your phone.
