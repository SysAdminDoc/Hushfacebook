...
* **Facebook:** If the gallery won't remove an unfinished save, Hushfacebook remembers it and tries again at the first save after Facebook restarts.
...
* **Facebook:** The Hushfacebook item now stays first when you long-press Facebook's icon. Facebook pushes three shortcuts of its own ahead of it whenever it posts notifications, and a launcher that shows only a few shortcuts, or fewer next to a notification, left Hushfacebook off the end (#2).
...
* **Facebook:** New patch, Block background-return feed refresh. It keeps the feed in place when you return to Facebook within ten minutes. Pull to refresh and a fresh launch still work. The patch starts off in Morphe Manager, and its switch starts on once you pick it.
