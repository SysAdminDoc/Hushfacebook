...
.EXAMPLE
    ...
    The clean APK defaults to the 46.2.3 vendor build in the folder HUSHFACEBOOK_FIXTURE_DIR names.
#>
[CmdletBinding()]
param(
    [string]$CleanApk,
...
if (-not $DesktopJar -or -not (Test-Path -LiteralPath $DesktopJar -PathType Leaf)) {
    ...
    throw 'No Morphe desktop CLI. Pass -DesktopJar or set HUSHFACEBOOK_DESKTOP_JAR.'
}
...
if (-not $CleanApk -and $env:HUSHFACEBOOK_FIXTURE_DIR) {
    $fixture = Get-ChildItem -LiteralPath $env:HUSHFACEBOOK_FIXTURE_DIR -Filter '*46.2.3*.apk' -File -ErrorAction SilentlyContinue |
        Select-Object -First 1
...
# because a full Facebook is hundreds of megabytes. Worked out before the default path is filled
